...
static void MSMigrateLegacyDirectory(void) {
    ...
    NSString *legacy = [NSHomeDirectory()
        stringByAppendingPathComponent:@"Library/Application Support/PersonalTools"];
...
static void MSWriteMicStatus(NSString *state,
                             ...
                             personaltools::EchoProfile echoProfile = personaltools::EchoProfile::adaptive,
                             const personaltools::EchoMetrics& echoMetrics = {},
    ...
    if (echoEnabled) {
        ...
        } else if (echoMetrics.convergence == personaltools::EchoConvergenceState::tracking) {
            echoState = @"tracking";
        } else if (echoMetrics.convergence == personaltools::EchoConvergenceState::learning) {
            echoState = @"learning";
        } else {
            echoState = @"active";
        }
    ...
    NSDictionary *status = @{
        ...
        @"source": @"M2 Input 1",
        @"sourceUID": sourceUID ?: @"",
        @"virtualDeviceUID": @"io.griplabs.personaltools.mic.device",
        ...
        @"echoCancellationProfile":
            [NSString stringWithUTF8String:personaltools::echoProfileName(echoProfile)],
        ...
        @"echoConvergenceState":
            [NSString stringWithUTF8String:personaltools::echoConvergenceName(
...
static AudioDeviceID MSFindFallbackInputDevice(NSString *excludedSourceUID) {
    ...
    for (AudioObjectID device : devices) {
        ...
        NSString *uid = MSStringProperty(device, kAudioDevicePropertyDeviceUID);
        if ([uid isEqualToString:@"io.griplabs.personaltools.mic.device"] ||
...
static void MSSelectFallbackInputIfNeeded(NSString *excludedSourceUID) {
    ...
    if (current != kAudioObjectUnknown &&
        ![currentUID isEqualToString:@"io.griplabs.personaltools.mic.device"]) {
        return;
    }
...
@implementation MSEngine {
    ...
    std::unique_ptr<personaltools::StereoDSP> _dsp;
    ...
    std::unique_ptr<personaltools::StereoReferenceTimeline> _referenceTimeline;
    std::unique_ptr<personaltools::EchoCanceller> _echoCanceller;
    std::atomic<bool> _echoCancellationEnabled;
    std::atomic<bool> _echoResetRequested;
    personaltools::EchoProfile _echoProfile;
    uint64_t _micReferenceGeneration;
    std::unique_ptr<personaltools::EchoDelayMonitor> _micDelayMonitor;
    personaltools::MicRecoveryPolicy _micRecoveryPolicy;
    personaltools::MicRateRecovery _micRateRecovery;
...
- (void)setupStatusItem {
    ...
    _statusItem.button.image.accessibilityDescription = @"PersonalTools";
    _statusItem.button.toolTip = @"PersonalTools";

    _statusMenu = [[NSMenu alloc] initWithTitle:@"PersonalTools"];
...
- (void)buildStatusMenuItems {
    _engineStatusItem = [[NSMenuItem alloc] initWithTitle:@"PersonalTools · 상태 확인 중"
...
- (void)refreshStatusMenu {
    ...
    _engineStatusItem.title = [NSString stringWithFormat:@"PersonalTools · %@ · %@%@",
...
- (NSURL *)outerApplicationURL {
    ...
    while (url.path.length > 1) {
        if ([url.lastPathComponent isEqualToString:@"PersonalTools.app"]) return url;
...
- (void)menuOpenSettings:(id)sender {
    ...
    if (!applicationURL) {
        _menuError = @"PersonalTools 설정 앱을 찾지 못했습니다.";
        return;
    }
...
- (void)start {
    ...
    _micDelayMonitor = std::make_unique<personaltools::EchoDelayMonitor>();
    ...
    [self setMicHealthState:@"waiting" message:@"자동 복구: 스피커 신호를 기다리는 중입니다."
                     warning:NO];
    _referenceTimeline = std::make_unique<personaltools::StereoReferenceTimeline>();
    ...
    if (permission == AVAuthorizationStatusNotDetermined) {
        ...
        MSWriteMicStatus(@"permission required",
                         @"PersonalTools Engine의 마이크 접근 승인을 기다리는 중입니다.",
            ...
            [AVCaptureDevice requestAccessForMediaType:AVMediaTypeAudio
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (granted) {
                        ...
                        MSWriteMicStatus(@"permission required",
                                         @"시스템 설정 > 개인정보 보호 및 보안 > 마이크에서 PersonalTools Engine을 허용하세요.",
...
- (void)applyEchoSettingsWithoutRestart:(NSMutableDictionary *)updatedConfig {
    ...
    const personaltools::EchoProfile profile =
        personaltools::echoProfileFromName(profileName.UTF8String);
    ...
    if (_micRunning) {
        if (_micDevice == kAudioObjectUnknown || MSFindDevice(_micSourceUID) == kAudioObjectUnknown) {
            ...
            MSWriteMicStatus(@"source disconnected",
                             @"PersonalTools Mic을 내리고 M2 연결을 기다리는 중입니다. 연결되면 자동으로 복원합니다.",
                             _micSourceUID, 0, _micShared);
            return;
        }
        if (_micNeedsDefaultSelection) {
            const AudioDeviceID virtualMic = MSFindDevice(@"io.griplabs.personaltools.mic.device");
        ...
        const personaltools::EchoMetrics echoMetrics = _echoCanceller
            ? _echoCanceller->metrics(_referenceTimeline ? _referenceTimeline->underruns() : 0)
            : personaltools::EchoMetrics{};
...
- (void)monitorMicHealth:(BOOL)echoEnabled {
    ...
    if (_micRecoveryVerification) {
        if (decision == personaltools::MicHealthDecision::healthy) ++_micGoodMeasurements;
    ...
    switch (decision) {
        case personaltools::MicHealthDecision::recover:
            [self beginMicRecovery:observation.delayMs];
            break;
        case personaltools::MicHealthDecision::warning: {
        ...
        case personaltools::MicHealthDecision::limited: {
...
- (void)micRecoveryTick:(NSTimer *)timer {
    ...
    if (state == personaltools::MicRateRecovery::State::captureStarted) {
...
- (void)reconcileMic {
    ...
    NSString *profileName = @"adaptive";
    _echoProfile = personaltools::echoProfileFromName(profileName.UTF8String);
    ...
    if (device == kAudioObjectUnknown || MSChannelCount(device, kAudioDevicePropertyScopeInput) < 1) {
        ...
        MSWriteMicStatus(@"source disconnected",
                         @"M2 입력 1을 찾지 못했습니다. PersonalTools Mic은 숨겨지고 다른 입력이 기본값이 됩니다.",
    ...
    _echoCanceller = std::make_unique<personaltools::EchoCanceller>();
    ...
    if (virtualMic != kAudioObjectUnknown) {
        ...
        message = @"PersonalTools Mic 장치가 게시되기를 기다리는 중입니다.";
    }
...
- (void)reconcile {
    ...
    std::unique_ptr<personaltools::StereoDSP> dsp;
    if (eqEnabled) {
        const auto left = personaltools::parseREWConfigurablePEQFile(
            std::filesystem::path(MSFilterPath(YES).fileSystemRepresentation),
            personaltools::Channel::left);
        const auto right = personaltools::parseREWConfigurablePEQFile(
            std::filesystem::path(MSFilterPath(NO).fileSystemRepresentation),
            personaltools::Channel::right);
        if (!left || !right) {
        ...
        dsp = std::make_unique<personaltools::StereoDSP>();
        std::string dspError;
    ...
    tapDescription.name = @"PersonalTools Output Reference Tap";
    ...
    if (status != noErr || _tap == kAudioObjectUnknown) {
        NSString *message = [NSString stringWithFormat:
            @"오디오 탭 생성 실패: %@. 시스템 설정 > 개인정보 보호 및 보안 > 시스템 오디오 녹음에서 PersonalTools Engine 권한을 확인하세요.",
            MSOSStatusDescription(status)];
    ...
    NSString *aggregateUID = [NSString stringWithFormat:@"io.griplabs.personaltools.private.%@",
                                                        NSUUID.UUID.UUIDString];
    NSDictionary *aggregateDescription = @{
        @kAudioAggregateDeviceNameKey: @"PersonalTools Private EQ Route",
...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        const int singletonDescriptor = open("/tmp/io.griplabs.personaltools.engine.lock",
