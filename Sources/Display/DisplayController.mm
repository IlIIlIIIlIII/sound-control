...
NSString *const MTDisplaySwapNotification = @"io.griplabs.personaltools.display.swap";
...
NSString *MTBaseDirectory(void) {
    return [NSHomeDirectory() stringByAppendingPathComponent:
        @"Library/Application Support/PersonalTools"];
}
...
NSData *MTDesiredEDID(NSString **error) {
    ...
    if (!data || !personaltools::display::validateEDID(bytes, validationError) ||
        !personaltools::display::isTarget32RTX950EDID(bytes)) {
        if (error) {
...
NSArray<NSNumber *> *MTOrderedOnlineExternalDisplayIDs(BOOL *hasBuiltIn) {
    ...
    BOOL builtin = NO;
    std::vector<personaltools::display::ReinitializeDisplay> external;
    ...
    for (const auto display : personaltools::display::makeReinitializeOrder(
             std::move(external))) {
...
std::vector<personaltools::display::DisplayGeometry> MTDisplayGeometries(void) {
    uint32_t count = 0;
    std::vector<personaltools::display::DisplayGeometry> result;
    if (CGGetOnlineDisplayList(0, nullptr, &count) != kCGErrorSuccess || count == 0) {
        return result;
    }
    std::vector<CGDirectDisplayID> ids(count);
...
- (NSDictionary *)inspectEDIDApplying:(BOOL)apply {
    ...
    while ((service = IOIteratorNext(iterator)) != IO_OBJECT_NULL) {
        ...
        if (avService && copy(avService, &current) == kIOReturnSuccess && current) {
            ...
            if (exact || personaltools::display::isTarget32RTX950EDID(bytes)) {
                ++matched;
                if (apply && !exact) {
...
- (void)swapMO32U24Displays {
    ...
    auto plan = personaltools::display::makeMO32U24SwapPlan(MTDisplayGeometries(), planError);
    if (!plan) {
...
- (NSDictionary *)diagnosticSnapshot {
    ...
    const auto swapPlan = personaltools::display::makeMO32U24SwapPlan(geometries, planError);
    return @{
        @"edid": edid,
